...
function AdminPage() {
  // Asegura que sólo renderiza en /ajustes
  ...
  // Decodificar token para saber si es Anette
  useEffect(() => {
    if (!token) return;
    try {
  ...
  const rechazar = (c) => callAccion(c, 'rechazar'); // para PENDIENTE
  const revocar  = (c) => callAccion(c, 'revocar');  // para APROBADO
